% 本程序由于求解高斯点应力，同时采用高斯点应变推求节点应变
function [strain,stress]=stresstrainop2(dmat,dispx,dispy,guassnode,totbmart,nnode)
% 数组初始化
[selem,selen]=size(dispx);
elementdisp=zeros(1,8); 
strain=zeros(nnode*selem,3); stress=zeros(nnode*selem,3);
for i=1:selem
    % 单元节点的x，y方向的位移
    ...
    % 建立高斯点存储
    gstrain=zeros(4,3); gstress=zeros(4,3);
    for j=1:selen
        % 单元节点高斯点的应变strain=b*disp
        estrain=estrain2(3*(j-1)+(1:3),:);
        % 单元高斯点应力stress=d*strain
        estress=dmat*estrain;
    ...
    dstress=guassnode*gstress;
    strain(4*(i-1)+(1:4),:)=dstrain;
    stress(4*(i-1)+(1:4),:)=dstress;
end