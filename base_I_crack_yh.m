...
str0='base-3';   %%--%%--%% 输入计算网格名
storefile='I_crack_yh2';  %%--%%--%% 输入存储结果的文件
str2='\';  str3='.data'; 
strfile=[str0 '.txt'];
...
for st1=1:step
    ...
    elseif st1<=600 
        ...
        loadstep(st1,1)=0;  %%--%%--%%--%% 输入x方向的荷载
        loadstep(st1,2)=0.25e-8; %%--%%--%%--%% 输入y方向的荷载      
    end    
end
% 荷载步综合 1为x方向 2为y方向
...
for st1=1:step
    ...
    tic
    % 更新相场值和力场值
    [totalph,totalstress,totaldisp,totalhm,loadforcest,pristress,pristrain]=renewresult_yh ...
      (snode,degssor,xdetaf,xdetap,sudisp,totaldisp,totalph,totalhm,element,dmat,guassnode, ...
       totbmart,nnode,linknode,foreleminf,lemada,meu,modulus,poisson);
...
%% 后处理及输出文件
% 输出荷载反力
strout=[storefile str2  '荷载-位移'  str3];
