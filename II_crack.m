...
u1=1; u2=0; %%--%%--%%--%% 输入 u1为x方向，u2为y方向，0为约束 其他为未约束
dispnum=dispnum+1; dispxy(dispnum,:)=[x1 x2 y1 y2 0 u1 u2];
...
% 选择节点信息
x1=lengxs; x2=lengx; y1=lengy; y2=lengy;   %%--%%--%%--%% 输入
u1=0; u2=1; %%--%%--%%--%% 输入 u1为x方向，u2为y方向，0为施加位移荷载
...
for st1=1:step
    if st1<=200
        loadstep(st1,1)=3e-8;  %%--%%--%%--%% 输入x方向的荷载
        loadstep(st1,2)=0; %%--%%--%%--%% 输入y方向的荷载
    elseif st1<=500 
        loadstep(st1,1)=2e-8;  %%--%%--%%--%% 输入x方向的荷载
        loadstep(st1,2)=0; %%--%%--%%--%% 输入y方向的荷载
    else
        loadstep(st1,1)=1e-8;  %%--%%--%%--%% 输入x方向的荷载
        loadstep(st1,2)=0; %%--%%--%%--%% 输入y方向的荷载
    end    
end
% 荷载步综合 1为x方向 2为y方向
