...
str0='crack';   %%--%%--%% 输入计算网格名
storefile='I_crack_0.5';  %%--%%--%% 输入存储结果的文件
str2='\';  str3='.vtk'; 
strfile=[str0 '.txt'];
...
% nodecoor节点坐标
nodecoor=getdata(2:snode+1,2:3)/1000+0.5e-3;
% element单元所含的节点
...
% 下端ux,uy等于0
x1=0; x2=lengx; y1=0; y2=0;  %%--%%--%%--%% 输入
u1=1; u2=0; %%--%%--%%--%% 输入 0为约束 其他为未约束
dispnum=dispnum+1; dispxy(dispnum,:)=[x1 x2 y1 y2 0 u1 u2];

% 输入信息
% 左侧ux等于0
x1=0; x2=0; y1=0; y2=lengy;  %%--%%--%%--%% 输入
u1=1; u2=1; %%--%%--%%--%% 输入 u1为x方向，u2为y方向，0为约束 其他为未约束
dispnum=dispnum+1; dispxy(dispnum,:)=[x1 x2 y1 y2 0 u1 u2]; 

% 输入信息
% 右侧ux等于0
x1=lengx; x2=lengx; y1=0; y2=lengy;  %%--%%--%%--%% 输入
u1=1; u2=1; %%--%%--%%--%% 输入 u1为x方向，u2为y方向，0为约束 其他为未约束
dispnum=dispnum+1; dispxy(dispnum,:)=[x1 x2 y1 y2 0 u1 u2];

% 输入信息
% 上端ux等于0
x1=0; x2=lengx; y1=lengy; y2=lengy;  %%--%%--%%--%% 输入
u1=1; u2=1; %%--%%--%%--%% 输入 u1为x方向，u2为y方向，0为约束 其他为未约束
dispnum=dispnum+1; dispxy(dispnum,:)=[x1 x2 y1 y2 0 u1 u2];
...
for st1=1:step
    ...
    if adapstart==1   
        ...
        outputvtk(strout,snode,selement,nodecoor,element,totalph,totaldisp);
    ...
    % 计算位移荷载对应的单元矩阵
    [msdisp]=loadmart(dispelem,element,nodecoor,totalph,nnode, ...
    ...
    loadforce(st1,1)=sum(totalstress(foreleminf(:,2),1).*foreleminf(:,3)/2/1000);
    loadforce(st1,2)=sum(totalstress(foreleminf(:,2),2).*foreleminf(:,3)/2/1000); 
    ...
    %% 剖分条件判断，条件设置为标记为0单元的节点最大值max(phi)>=0.8 
    % 选择0单元的节点
    elemark_n0=elemark~=0;
    nodemark_n0=unique(element(elemark_n0,:));
    nodeall=1:1:snode;
    nodeall(nodemark_n0)=[];
    nodemark_0=nodeall;
    % 设置剖分变量
    totalph_0=totalph(nodemark_0);  
    
    if  max(totalph_0)>=0.5
...
%         if adapstep==1
%            % 调整网格
%            tonoel=[snode selement];
%            fileid=fopen('mesh_ts.txt','wt');
%            formatSpec=' %5d  %5d\n';
%            fprintf(fileid,formatSpec,tonoel');
%            LE2=[nodecoor totalph];
%            formatSpec='%5d   %12.7f  %12.7f\n';
%            fprintf(fileid,formatSpec,LE2');
%            LEnew=[element elemark];
